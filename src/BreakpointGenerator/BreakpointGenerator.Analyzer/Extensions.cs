...
namespace Microsoft.ALMRangers.BreakpointGenerator.Analyzer
{
    ...
    public static class Extensions
    {
        public static IEnumerable<EntryPoint> GetPublicEntryPoints(this ClassDeclarationSyntax c)
        {
            ...
            foreach (var m in c.DescendantNodes().OfType<MethodDeclarationSyntax>().Where(m => m.Modifiers.Any(mod => mod.Text.Equals("public"))))
            {
                entryPoints.Add(new EntryPoint(m));
            }
            foreach (var m in c.DescendantNodes().OfType<ConstructorDeclarationSyntax>().Where(m => m.Modifiers.Any(mod => mod.Text.Equals("public"))))
            {
                entryPoints.Add(new EntryPoint(m));
            }
            foreach (var m in c.DescendantNodes().OfType<PropertyDeclarationSyntax>().Where(m => m.Modifiers.Any(mod => mod.Text.Equals("public"))))
            {
